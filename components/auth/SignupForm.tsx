...
export default function SignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: authError } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { name, avatar_url: `avatar:${avatar.id}:${avatar.bg}` } },
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <Input id="name" label="Full Name" type="text" placeholder="Necdet Kerem"
      ...
      <Button type="submit" loading={loading} className="w-full" size="lg" disabled={!agreed}>
        Create account
      </Button>
      ...
      <Button type="button" variant="ghost" size="lg" loading={googleLoading} onClick={handleGoogle} className="w-full">
